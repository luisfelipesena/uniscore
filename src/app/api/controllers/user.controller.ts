...
export async function esqueciMinhaSenha(req: Request, res: Response) {
  ...
  try {
    ...
    if (usuario) {
      ...
      await pool.query(
        'INSERT INTO pedidos_redefinicao_senha (email, token) VALUES ($1, $2)',
        [email, tokenRedefinicaoSenha]
      );
      ...
      transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
          ...
          res.status(500).json({
            mensagem:
              'Erro interno do servidor ao enviar o e-mail de redefinição de senha.',
          });
        } else {
          console.log(
            'E-mail de redefinição de senha enviado: ' + info.response
          );
          res.json({
            mensagem:
              'Um e-mail de redefinição de senha foi enviado para o seu endereço de e-mail registrado.',
          });
        }
      });
    } else {
